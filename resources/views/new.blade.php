    ...
    <div class="row py-5">
        <div class="col-9">
            ...
            <div class="row mb-4 d-flex align-items-center">
                ...
                <div class="col-8">
                    <div>
                        <a href="{{route('new_detail',$item->id)}}" class="h5 text-dark">{{$item->title}}</a>
                        <p class="text-secondary mt-2">{{$item->description}}</p>
                        <h6 style="font-size: 13px;">{{$item->created_at->format('d/m/Y') . ' bởi ' .
                            $item->user->name}} - <i class="fas fa-eye"></i> {{$item->views ? $item->views : 0}}</h6>
                        <a href="{{route('new_detail',$item->id)}}" class="btn btn-outline-dark mt-2">Xem chi tiết</a>
                    </div>
                </div>
            </div>
            @endforeach
