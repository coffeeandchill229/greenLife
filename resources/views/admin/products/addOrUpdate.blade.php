<x-admin title="{{$product_edit ? 'Cập nhật' : 'Thêm'}} sản phẩm">
    <form action="{{route('admin.product.store',$product_edit ? $product_edit->id : '')}}" method="post" enctype="multipart/form-data">
        <div class="row">
            @csrf
            <div class="col-6">
                ...
                <x-input name="stock" type="number" label="Tồn kho" value="{{$product_edit ? $product_edit->stock : ''}}"/>
                <div class="form-group">
                    <label class="form-label">Mô tả</label>
                    <textarea name="description" class="form-control" rows="3">{{$product_edit ? $product_edit->description : old('description')}}</textarea>
                    @error('description')
