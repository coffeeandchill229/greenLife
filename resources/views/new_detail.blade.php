<x-only-header title="{{ $new_detail->title }}">
    ...
    <div class="row py-5">
        <div class="col-lg-9 col-12">
            ...
            <p class="mt-3">Đăng vào <span class="text-success">{{ $new_detail->created_at->format('d/m/Y') }}</span>
                bởi
                <span class="text-success">{{ $new_detail->user->name }}</span>
            </p>
            <div class="mt-3 new_content" style="text-align: justify;">
                {!! $new_detail->content !!}
            </div>
            {{-- Comment --}}
                        ...
                        <div class="ms-2" style="width: 100%;">
                            ...
                            @if ($item->replies)
                                @foreach ($item->replies as $child)
                                    <div class="row">
                                        <div class="d-flex">
                                            <img src="/storage/avatars/{{ $child->customer->avatar }}" width="40"
                                                height="40" class="rounded-circle" alt="">
                                            <div class="ms-2" style="width: 100%;">
                                                <h6>{{ $child->customer->name }}</h6>
                                                <p class="p-2 rounded-2"
                ...
                <div class="d-flex mt-3">
                    ...
                    <div class="ms-2" style="width: 100%;">
                        <form action="{{ route('home.store_comment', [$new_detail->id, Auth::user()->id]) }}"
                            ...
                            <div class="text-end mt-3">
                                <button class="btn btn-sm btn-dark">Gửi</button>
                            </div>
                        </form>
                    </div>
                </div>
